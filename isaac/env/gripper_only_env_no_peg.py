import sys
sys.path.append("/home/cyx/.local/share/ov/pkg/isaac_sim-2023.1.1/exts/omni.isaac.robot_assembler")
import numpy as np
...
class env:
    ...
    def sample(self):
        ...
        self.ref_camera_trans_matrix = self.desired_trans_matrix_to_hole @ np.linalg.inv(self.target_in_hand_error_matrix) @ self.camera_to_end_effector_trans_matrix
    ...
    def get_observation(self):
        if self.count > 10:
            ...
            delta_camera_trans_matrix = np.linalg.inv(self.cur_camera_trans_matrix) @ desired_camera_trans_matrix
            delta_pos = delta_camera_trans_matrix[:3, 3]
            delta_ori = rot_matrix_to_quat(delta_camera_trans_matrix[:3, :3])
            delta_ori = quat_to_euler_angles(delta_ori, extrinsic=False)
            # print(delta_pos, delta_ori)
            print(np.linalg.norm(delta_pos), np.linalg.norm(delta_ori))
            ...
            hole_desired_feature, peg_desired_feature = self.feature.project_points_to_img(self.ref_camera_trans_matrix, cur_hole_trans_matrix, cur_hole_trans_matrix @ self.desired_trans_matrix_to_hole)
            print(np.linalg.norm(vel))
            print(cur_peg_trans_matrix, self.desired_trans_matrix_to_hole)
